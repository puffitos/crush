...
func init() {
	// Only fall back to build info when Version wasn't set via -ldflags
	// (e.g. `go install ...@latest`). Go 1.24+ synthesizes a VCS-derived
	// pseudo-version for plain `go build`, which would otherwise clobber
	// the ldflags value.
	if Version == "devel" {
